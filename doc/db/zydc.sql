...
CREATE TABLE `z_session` (
  ...
  `user_name` varchar(20) DEFAULT NULL COMMENT '供应商登录名',
  `session_id` varchar(32) DEFAULT NULL COMMENT '随机字符串',
  `created_at` datetime DEFAULT NULL COMMENT '创建时间',
  `expires_in` datetime DEFAULT NULL COMMENT '过期时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='服务端session表，验证用户登录是否过期';
